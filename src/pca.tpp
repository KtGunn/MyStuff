    ...
    BoxQ k_SimpleBoxQ (typename pcl::PointCloud<PointT>::Ptr cluster)
{
    ...
    qBox.bboxTransform = Eigen::Vector3f ({(vPs[1].x + vPs[0].x)/2,(vPs[1].y + vPs[0].y)/2,(vPs[1].z + vPs[0].z)/2});
...
{   
    ...
    if (false) {
	    ...
	    std::cout << "  2nd eigenV = " << eigenM.col(1)[0] << " " <<eigenM.col(1)[1] << " " <<eigenM.col(1)[2] << std::endl;
	    std::cout << "   3rd eigenV = " << eigenM.col(2)[0] << " " <<eigenM.col(2)[1] << " " <<eigenM.col(2)[2] << std::endl;
	    std::cout << " EigenVals = " << eigenVs[0] << " " << eigenVs[1] << " " << eigenVs[2] << std::endl;
    }
    ...
    // DEBUGGING         
    // The average of min/max must be the center of the box, not the centroid
    Eigen::Vector3f vAvg  ({(vPts[1].x + vPts[0].x)/2,(vPts[1].y + vPts[0].y)/2,(vPts[1].z + vPts[0].z)/2 });
    ...
    // Must add the average(min+max) to the centroid and set to center of BoxQ
    Eigen::Vector3f newT = qB.bboxQuaternion * vAvg + qB.bboxTransform;
    qB.bboxTransform = newT;
